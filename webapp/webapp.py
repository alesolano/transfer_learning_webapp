...
def training():
    ...
    if request.method == 'POST':
        ...
        #model_name = request.form['ml_models'] # it returns 'on' instead of 'inception_resnet_v2'
        ...
        if file and allowed_file(file.filename):
            ...
            return redirect(url_for('uploaded', filename=filename, retrained_model_name=retrained_model_name,
                imagenet_model_name=model_name, classes=classes_list))

@app.route('/uploaded')
def uploaded():
    filename = request.args.get('filename')
    retrained_model_name = request.args.get('retrained_model_name')
    imagenet_model_name = request.args.get('imagenet_model_name')
    classes = request.args.getlist('classes')
    print("Selected model: " + retrained_model_name + ' - ' + imagenet_model_name)
    print("Classes:", classes)
    print("Filename:", filename)

    from predicting_transfer import predict

    begin = time.time()
    pred_class, pred_score = predict(
        filename=filename,
        retrained_model_name=retrained_model_name,
        imagenet_model_name=imagenet_model_name,
        classes=classes)
    end = time.time()
